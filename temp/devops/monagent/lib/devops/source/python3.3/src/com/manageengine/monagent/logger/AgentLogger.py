...
# Log level constants found in logging module.
# CRITICAL = 50; FATAL = CRITICAL; ERROR = 40; WARNING = 30; WARN = WARNING; INFO = 20; DEBUG = 10; NOTSET = 0
# Log level 12 prints request and url stuff. 
...
def initialize(str_logConfFilePath, str_logDirectory):
    global LOG_DIR
    ...
    try:
        ...
        pass
    debug(STDOUT,'Python version : '+repr(sys.version_info))
    
def createLogger(logInfo):
    logger = logging.getLogger(logInfo.getLogName())    
    hdlr = logging.handlers.RotatingFileHandler(logInfo.getLogFilePath(), maxBytes=logInfo.getLogFileSize(), backupCount=logInfo.getLogFileCount())
...
def is_log_eligibile(case):
    if case in AgentConstants.LOG_MESSAGE_TIMER and (time.time() - AgentConstants.LOG_MESSAGE_TIMER[case][1] >= AgentConstants.LOG_MESSAGE_TIMER[case][0]):
        AgentConstants.LOG_MESSAGE_TIMER[case][1] = time.time()
        return True
    else:
        return False
...
def log(loggerName, message,encrypt=False):
    try:
        if not AgentConstants.STOP_LOGGING:
            message = re.sub('(CUSTOMERID|custID|apikey|deviceKey|key|MSPCUSTOMERID)(=)([^&]*)',r'\1\2!@#$%',message)
            if encrypt:
                message = AgentCrypt.encrypt_with_ss_key(message)
...
def logonce(case, loggername, message):
    try:
        if is_log_eligibile(case):
...
def warning(loggerName, message):
    try:
        if type(loggerName) is list:
            for logName in loggerName:                
                logger = logging.getLogger(logName)        
...
def critical(loggerName, message):
    try:
        if type(loggerName) is list:
            for logName in loggerName:                
                logger = logging.getLogger(logName)        
