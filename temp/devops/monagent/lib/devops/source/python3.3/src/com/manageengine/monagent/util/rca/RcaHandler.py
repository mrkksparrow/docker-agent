...
def backupRCAReport(directory):
    AgentLogger.log(AgentLogger.MAIN,'Backing up RCA files from the directory '+directory)
    list_files = FileUtil.getSortedFileList(directory, str_loggerName=AgentLogger.STDOUT)
    for fileName in list_files[-10:]:
        FileUtil.copyFile(fileName, AgentConstants.AGENT_TEMP_RCA_REPORT_BACKUP_DIR)
...
class RcaReportHandler:
        ...
        try:
            if list_rcaFilesForCalculatingAverage:
                for str_filePath in list_rcaFilesForCalculatingAverage:
                    ...
                    #AgentLogger.log(AgentLogger.COLLECTOR,'PROCESS INFO --- {0}'.format(dict_processIdVsProcessDetails))
                    if bool_toReturn:
                        if list_processMetrics:
                            for index, list_processDetail in enumerate(list_processMetrics):
                                #AgentLogger.log(AgentLogger.COLLECTOR,'#### list process detail -- {0}'.format(list_processDetail))
                                ...
                                if str_processId in dict_processIdVsProcessDetails:
                                    ...
                                    #AgentLogger.log(AgentLogger.COLLECTOR,'### proc list -- {0}'.format(list_process))
                                    list_processDetail[2] = float(float(list_processDetail[4]) + float(list_process[4])) #float_psCpu
                                    list_processDetail[3] = float(float(list_processDetail[5]) + float(list_process[5])) #float_psMem
                                    list_processDetail[8] = float(float(list_processDetail[10]) + float(list_process[10])) #float_topCpu
                                    list_processDetail[9] = float(float(list_processDetail[11]) + float(list_process[11])) #float_topMem
                                    list_processDetail[4] = float(float(list_processDetail[6]) + float(list_process[6])) #float_psThreadCount
                                    list_processDetail[5] = float(float(list_processDetail[7]) + float(list_process[7])) #float_psHandleCount
                                else:
                                    #AgentLogger.log(AgentLogger.COLLECTOR,'Details for process Id : '+repr(list_processDetail))
                                    #('12917', ['12917', 'www-data', '19', 'apache2', '0.0', '0.0', '27', '11', '/usr/sbin/apache2', '/usr/sbin/apache2 -k start', '0.0', '0.0', '-k'])
                                    list_processDetail[2] = float(list_processDetail[4]) #float_psCpu
                                    list_processDetail[3] = float(list_processDetail[5]) #float_psMem
                                    list_processDetail[8] = float(list_processDetail[10]) #float_topCpu
                                    list_processDetail[9] = float(list_processDetail[11]) #float_topMem
                                    list_processDetail[4] = float(list_processDetail[6]) #float_psThreadCount
                                    list_processDetail[5] = float(list_processDetail[7]) #float_psHandleCount
                        else:
                            list_processMetrics = []
                ...
                for list_processDet in list_processMetrics:
                    ...
                    list_processDet[2] = "%.1f" % float(float(list_processDet[4])/int_noOfRCAFilesTakenForAverage) #float_psCpuAvg
                    list_processDet[3] = "%.1f" % float(float(list_processDet[5])/int_noOfRCAFilesTakenForAverage) #float_psMemAvg
                    list_processDet[8] = "%.1f" % float(float(list_processDet[10])/int_noOfRCAFilesTakenForAverage) #float_topCpuAvg
                    list_processDet[9] = "%.1f" % float(float(list_processDet[11])/int_noOfRCAFilesTakenForAverage) #float_topMemAvg
                    list_processDet[4] = "%.1f" % float(float(list_processDet[6])/int_noOfRCAFilesTakenForAverage) #float_psThreadCountAvg
                    list_processDet[5] = "%.1f" % float(float(list_processDet[7])/int_noOfRCAFilesTakenForAverage) #float_psHandleCountAvg
                if list_processMetrics:
                    dict_toReturn = {}
    ...
    def calculateOSXTopProcessMetricsAvgForRca(self, list_rcaFilesForCalculatingAverage):
        dict_toReturn = None
        list_processMetrics = None
        try:
            if list_rcaFilesForCalculatingAverage:
    ...
    def fetchAvgProcessMetricsForRca(self):
        try:
            ...
            if AgentConstants.OS_NAME == AgentConstants.FREEBSD_OS:
                ...
                dict_topProcessMetricsAvgForRCA = self.calculateOSXTopProcessMetricsAvgForRca(list_rcaFilesForCalculatingAverage)                
    ...
    def generateRca(self, rcaInfo):
        ...
        try:
            ...
            if top_process_data:
                dict_rcaMetrics['Top Process Report']['top.process.cpu']=top_process_data['TOPCPUPROCESS']
                dict_rcaMetrics['Top Process Report']['top.process.mem']=top_process_data['TOPMEMORYPROCESS']
            dict_collectedData = COLLECTOR.collectData(COLLECTOR.getMonitorsGroup()['MonitorGroup']['RootCauseAnalysis'])
            dict_collectedData = {} if dict_collectedData is None else dict_collectedData
    ...
    def getRCAFileFromBackup(self,rcainfo):
        list_rcaReportFiles = []
        list_files = None
        list_files = FileUtil.getSortedFileList(AgentConstants.AGENT_TEMP_RCA_REPORT_BACKUP_DIR, str_loggerName=AgentLogger.STDOUT)
        if not list_files:
            ...
            if not list_rcaReportFiles:
                list_rcaReportFiles = self.getRcaFileList(rcainfo)
        AgentLogger.log(AgentLogger.STDOUT, 'List of rca report files from backup : '+repr(self.getFormattedFileList(list_rcaReportFiles)))
    ...
    def uploadRca(self, rcaInfo):
        ...
        try:  
            ...
            if rcaInfo.requestType != AgentConstants.RCA_REPORT:
                list_rcaReportFiles = self.getRCAFileFromBackup(rcaInfo)
            ...
            if rcaInfo.requestType == AgentConstants.GENERATE_NETWORK_RCA:
                ...
                trace_dict = AgentUtil.getTraceRouteData(rcaInfo.searchTime)
                AgentLogger.log(AgentLogger.STDOUT, 'RCA search time : '+repr(rcaInfo.searchTime))
                AgentLogger.log(AgentLogger.STDOUT, 'Trace Route Data : '+repr(trace_dict))
                if not trace_dict == None:
                    dict_rcaData['TraceRoute'] = trace_dict
            else:
            ...
            AgentLogger.debug(AgentLogger.STDOUT,'RCA json data : '+repr(str_jsonData))
            str_url = None
                ...
                if rcaInfo.requestType==AgentConstants.GENERATE_NETWORK_RCA:
                    ...
                    for file in list:
                        if 'trace_route' in file:
                            ...
                            removeFile = AgentConstants.AGENT_TEMP_RCA_DIR+'/'+tail
                            AgentLogger.log(AgentLogger.CHECKS,'Trace Route File to be removed ---> '+repr(removeFile))
